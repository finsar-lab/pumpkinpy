...
@author: Héctor Quiroz (hectoryoramquiroz@gmail.com)
***Version 2025
"""
...
from pumpkinpy import clustering as KMeans_clustering
from pumpkinpy import pca as PCAnalysis
from pumpkinpy import regression as regression_DFT
from pumpkinpy import reprojection as V_H_reprojection
from pumpkinpy.config import load_config
...
# Original data
# ASC = pd.read_csv(raw_data_dir / 'ascending_InSAR_sample.csv', sep=',')
# DESC = pd.read_csv(raw_data_dir / 'descending_InSAR_sample.csv', sep=',')

asc_dataframe = pd.read_parquet(raw_data_dir / "asc_dataset.parquet")
desc_dataframe = pd.read_parquet(raw_data_dir / "desc_dataset.parquet")

datasets_names = ["data_asc", "data_desc"]

Dataset_asc_desc_dict = dict(zip(datasets_names, [asc_dataframe, desc_dataframe], strict=False))

def setup_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    coordinates = df[["X", "Y"]]

    date_cols = [col for col in df.columns if col.startswith("D2")]
    timestamps = df[date_cols]
    timestamps.columns = [pd.to_datetime(col[1:]) for col in date_cols]

    angles = df[["incidence_angle", "azimuth_angle"]]
    return pd.concat([coordinates, angles, timestamps], axis=1), timestamps

asc_dataframe, asc_timestamps = setup_dataframe(asc_dataframe)
desc_dataframe, desc_timestamps = setup_dataframe(desc_dataframe)

merged_lats = pd.concat([asc_dataframe["X"], desc_dataframe["X"]])
merged_longs = pd.concat([asc_dataframe["Y"], desc_dataframe["Y"]])
...
for key in Dataset_asc_desc_dict:
    data_coordinates[key] = pd.DataFrame(Dataset_asc_desc_dict[key], columns=["X", "Y"])
    ts[key] = Dataset_asc_desc_dict[key].loc[
        :, Dataset_asc_desc_dict[key].columns.str.startswith("D2")
    ]
    ...
    date_formatted[key] = np.array(
        [dt.datetime.strptime(d, "%Y%m%d").date() for d in date_ts[key]]
    )  # line for formatting date names
    ...
    df_coords[key] = pd.concat(
        [data_coordinates[key], angles_data[key], ts[key]], axis=1
    )
...
def process_cellsize(e, Tot_points, asc_df, desc_df):
    """Process a single cellsize value"""
    polygon_grid = V_H_reprojection.gridding(Tot_points, e)
    asc_within_grid, desc_within_grid = V_H_reprojection.grid_dissolve(
...
process_func = partial(
    ...
    asc_df=asc_dataframe, 
    desc_df=desc_dataframe
)
...
# gridsize.plot_knee()
plt.savefig(image_dir / "knee_plot.png")
...
optimal_grid = V_H_reprojection.gridding(total_points, best_grid_size)
...
asc_within_grid, desc_within_grid = V_H_reprojection.grid_dissolve(
    optimal_grid, asc_dataframe, desc_dataframe
)

interpolated_points = V_H_reprojection.resampling_ts(
    asc_timestamps, desc_timestamps, asc_within_grid, desc_within_grid
)
...
V_H_reprojection.reprojection(interpolated_points, optimal_grid)
...
for key in resampled_grid:
    Vv_df_array.append(resampled_grid[key]["Vv_TS"].to_numpy().reshape((1, -1)))
    Vh_df_array.append(resampled_grid[key]["Vh_TS"].to_numpy().reshape((1, -1)))
...
index_cells_geometry = [x for x in list(cell.index) if x in list(resampled_grid.keys())]
...
print("PCA")
df_array_std, stdarray_mean, stdarray_std = PCAnalysis.std_scaler(Vh_df_array)
df_array_std2, stdarray_mean2, stdarray_std2 = PCAnalysis.std_scaler(Vv_df_array)
...
kvh = KneeLocator(
    np.asarray([(range(len(perc_variance)))]).squeeze(),
    np.asarray([perc_variance]).squeeze(),
    curve="convex",
    direction="decreasing",
)
kvv = KneeLocator(
    np.asarray([(range(len(perc_variance2)))]).squeeze(),
    np.asarray([perc_variance2]).squeeze(),
    curve="convex",
    direction="decreasing",
)
...
(
    km,
    cluster_center,
    cluster_center_shape,
    time_series_class,
    labels,
    count_labels,
    inertia,
) = KMeans_clustering.Euclidean_KMeans(Vh_df_array, number_of_components)
(
    km2,
    cluster_center2,
    cluster_center_shape2,
    time_series_class2,
    labels2,
    count_labels2,
    inertia2,
) = KMeans_clustering.Euclidean_KMeans(Vv_df_array, number_of_components2)
...
df_coords_clusters = pd.concat(
    [
        geometry_cell.reset_index(drop=True),
        labels_dataframe.reset_index(drop=True),
        pd.DataFrame(index_cells_geometry, columns=["cell_index"]),
    ],
    axis=1,
)
df_coords_clusters2 = pd.concat(
    [
        geometry_cell.reset_index(drop=True),
        labels_dataframe2.reset_index(drop=True),
        pd.DataFrame(index_cells_geometry, columns=["cell_index"]),
    ],
    axis=1,
)
...
df_coords_clusters.to_file(outpath / "cluster_horizontal_components.shp")
df_coords_clusters2.to_file(outpath / "cluster_vertical_components.shp")
...
slope, intercept, rmse, powerspectrum = regression_DFT.LinRegression_DFT(
    new_dates, Vh_clusters_centroid_1
)
